...
```{r project.setup, message=FALSE, warning=FALSE, include=FALSE}
#Import libraries
...
```{r data.summary, echo=F, message=FALSE, warning=FALSE}
...
kable(summary_Ozone, col.names = c("County","$\\mu g/m^3$"), caption = "Ozone")
```
...
```{r plot, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Ozone in 2018", fig.align='left', fig.height= 2}
epa_ozone_2018 <- epa_data %>%
  filter(Year == 2018) %>% 
  drop_na()
  
 ggplot(data = epa_ozone_2018, aes (x = factor(Month), y = Ozone)) +
  geom_boxplot() +
  labs(
    x = "Month",
    y = "Ozone"
...
```
```{r plot2, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Ozone in 2018", fig.align='left', fig.height= 2}
epa_ozone_2018 <- epa_data %>%
  filter(Year == 2019) %>% 
  drop_na()
...
### Git Repository
https://github.com/jiz11/EDA-Spring2023.git
