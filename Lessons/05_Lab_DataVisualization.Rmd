...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(
    x=0,
    y=0.025,
    label=expression(paste("Mean is ", mu,",", R^{2},'=0.6')) #paste connect comma as spaces; paste0 output no spaces.
    )
...
factor(PeterPaul.chem.nutrients, levels = 1:12) # level fill missing month as factors
#Try again: Plot how many Lake observations occur each month
ggplot(PeterPaul.chem.nutrients, aes(x=factor(month))) + #factor() can make sure the x axes shows all the month, 
  geom_bar()
...
ggplot( 
       subset(PeterPaul.chem.nutrients, lakename == "Paul Lake" | lakename == "Peter Lake"),
       aes(x = daynum, y = temperature_C),
       color = year4) +
  geom_point() +
